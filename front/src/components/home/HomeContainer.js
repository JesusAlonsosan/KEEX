...
getProducts=()=>{
    ...
    if(user &&cat === 'all'){
        ...
        getAllProducts({id:user._id}).then(res=>this.setState({products:res.product})).catch(err=>console.log('error',err))
    }else{
         getByCategory(cat,{id:user._id}).then(res=>this.setState({products:res.products})).catch(err=>console.log('error',err))
    }
}
    render(){
       ...
       console.log(products)
        return(
            <div 
            style={{backgroundColor:'rgb(240, 235, 2)',padding:80,height:products.length?null:'90vh' }}>
            <CategoryBar onChange={handleChange} category={category}/>
            
            <div className="uuk-container uk-container-xpand"  style={{backgroundColor:'rgb(240, 235, 2)', width:'100%',padding:10 ,borderRadius:15, height:products.length?null:'600px' }}>
